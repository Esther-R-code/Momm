...
#' @param w_c category of Y that user have instercting in. If all in let wc="all",wc will set to be seq(1,n.cate,by=1).
#' @export Ord_mediation_analysis_pal_sc
#'
#' @return please see the help docunment of function "Ord_mediation_analysis_pal"
#'
Ord_mediation_analysis_pal_sc=function(Indata, n_cate, confounders, intv=4, intval=c(0, 1), nb=0, w_c, n_core=1){
  ...
  if(nb>0){
    for(cate.count in w_c){
      ...
      PSE_a<-PSEa_wiboot_c(databoot, Para=Para, n_para=n_para, n_cate=n_cate, theta.hat=theta.hat, sig.hat=sig.hat, Vcov.matrix=Vcov.matrix, intv=intv, intval=intval, confounders=confounders, a=cate.count, nb=nb, n_core=n_core)
      PSE<-cbind(PSE,PSE_a)#;print(PSE)
      coln_PSE<-c(coln_PSE,paste0("Outcome=",cate.count))#;print(coln_PSE)
    }
  }else{
   for(cate.count in w_c){
   	PSE_a<-PSEa_woboot(Para=Para, n_para=n_para, n_cate=n_cate, theta.hat=theta.hat, sig.hat=sig.hat, Vcov.matrix=Vcov.matrix, intv=intv, intval=intval, confounders=confounders, a=cate.count)
  	PSE<-cbind(PSE,PSE_a)#;print(PSE)
  	coln_PSE<-c(coln_PSE,paste0("Outcome=",cate.count))#;print(coln_PSE)
  	}
  }
